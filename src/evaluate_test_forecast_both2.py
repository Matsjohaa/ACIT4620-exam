...
def build_test_sample(train_df: pd.DataFrame, test_df: pd.DataFrame):
    ...
    day_ahead_MW = test_df["day-ahead"].values     # [336] baseline in MW
    inst_cap = test_df["installed_capacity_mw"].values

    dates = pd.to_datetime(test_df["date"].values)

    return X_enc_raw, X_dec_raw, y_true_MW, day_ahead_MW, inst_cap, dates, len(features)


def load_simple_model(n_features: int, device: torch.device) -> SimpleCNNLSTM:
    # CF-model trained WITHOUT residual
    ckpt_path = MODELS_DIR / "best_model_simple_cf.pt"
...
def load_encoder_model(n_features: int, device: torch.device) -> EncoderDecoderCNNLSTM:
    # CF-model trained WITHOUT residual
    ckpt_path = MODELS_DIR / "best_model_encoder_cf.pt"
    ckpt = torch.load(ckpt_path, map_location=device)
...
def plot_forecast(dates, y_true_MW, y_pred_MW, day_ahead_MW,
                  mae, rmse, r2, title_prefix: str, out_path: Path):
    plt.figure(figsize=(12, 4))
    plt.plot(dates, y_true_MW, label="Actual", linewidth=1.5)
    plt.plot(dates, day_ahead_MW, label="Day-ahead baseline", linestyle=":", linewidth=1.2)
...
def main():
    ...
    (X_enc_raw,
     ...
     inst_cap,
     dates,
     n_features) = build_test_sample(train_df, test_df)

    # -------- SIMPLE CF MODEL --------
    print("\n=== SIMPLE CF model on Oct 27–Nov 10 ===")
    mean_s, std_s = load_norm_params(MODELS_DIR / "norm_simple_cf.npz")
    ...
    with torch.no_grad():
        ...
        cf_pred_simple = simple_model(Xs).cpu().numpy().reshape(-1)  # [336]
    ...
    out_simple = RESULTS_DIR / f"{ZONE}_forecast_oct27_nov10_simple_cf.png"
    plot_forecast(
        dates,
        y_true_MW,
        y_pred_simple_MW,
        ...
        rmse_s,
        r2_s,
        f"{ZONE} – 14-Day Forecast (Simple CF)",
        out_simple,
    )

    # -------- ENCODER CF MODEL --------
    print("\n=== ENCODER CF model on Oct 27–Nov 10 ===")
    mean_e, std_e = load_norm_params(MODELS_DIR / "norm_encoder_cf.npz")
    ...
    with torch.no_grad():
        ...
        cf_pred_encoder = encoder_model(Xe, Xd).cpu().numpy().reshape(-1)
    ...
    out_encoder = RESULTS_DIR / f"{ZONE}_forecast_oct27_nov10_encoder_cf.png"
    plot_forecast(
        dates,
        y_true_MW,
        y_pred_encoder_MW,
        ...
        rmse_e,
        r2_e,
        f"{ZONE} – 14-Day Forecast (Encoder CF)",
        out_encoder,
    )
